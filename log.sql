...
SELECT id, destination_airport_id FROM flights WHERE year = 2020 and month = 7 and day = 29 and origin_airport_id = 8 ORDER BY hour DESC LIMIT 1;
...
SELECT passport_number FROM people WHERE phone_number IN (SELECT receiver FROM phone_calls WHERE year = 2020 and month = 7 and day = 28 and duration < 60) or phone_number IN (SELECT caller FROM phone_calls WHERE year = 2020 and month = 7 and day = 28 and duration < 60) and passport_number IN (SELECT passport_number FROM passengers WHERE flight_id IN (SELECT id FROM flights WHERE year = 2020 and month = 7 and day = 29 and origin_airport_id = 8 ORDER BY hour and minute DESC LIMIT 1));
...
and passport_number IN (SELECT passport_number FROM passengers WHERE flight_id IN (SELECT id FROM flights WHERE year = 2020 and month = 7 and day = 29 and origin_airport_id = 8 ORDER BY hour DESC LIMIT 1))
...
SELECT city FROM airports WHERE id IN (SELECT destination_airport_id FROM flights WHERE year = 2020 and month = 7 and day = 29 and origin_airport_id = 8 ORDER BY hour DESC LIMIT 1);